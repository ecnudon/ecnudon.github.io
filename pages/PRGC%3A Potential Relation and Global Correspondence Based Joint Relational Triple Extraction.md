...
- [[note]]
	...
	- Decoder比较有意思:
		...
		- ((62baf6dd-c95b-4cfa-9e06-f4f6328a7099))
			...
			- 没有使用LSTM-CRF, 而是只用了全连接层
				...
				- 损失函数:
					...
					- $i$是第$i$个token的特征向量在第$j$个关系下softmax之后的sub/obj的分数, y是tag
					- 分母的$n_r^{pot}$是这个句子被打上的所有可能的关系数量
		- ((62bafcb1-c4e3-432a-853e-7c12ca4cc4b6))
			...
			- 矩阵只看起始位置, 是不是有待改进
			- 损失函数:
				- ![image.png](../assets/image_1656425395433_0.png)
				- $i,\ j$都是token的下标,所以其实就是整个矩阵 该0的时候0, 该1的时候1
				- 看损失函数其实是一个句子一张表, 所有的关系都标在上面的.
	-